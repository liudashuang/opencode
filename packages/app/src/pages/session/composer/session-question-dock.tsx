...
export const SessionQuestionDock: Component<{ request: QuestionRequest; onSubmit: () => void }> = (props) => {
  ...
  const totalPicked = createMemo(() => {
    const n = store.answers[store.tab]?.length ?? 0
    if (n > 0) return n
    if (store.customOn[store.tab] !== true) return 0
    return (store.custom[store.tab] ?? "").trim().length > 0 ? 1 : 0
  })
  ...
  return (
    ...
    >
      <div
        ...
        classList={{
          "mb-6": store.collapsed && totalPicked() === 0,
        }}
      ...
      <Show when={store.collapsed && totalPicked() > 0}>
        <div data-slot="question-hint" class="cursor-default mb-6">
          {totalPicked()} answer{totalPicked() === 1 ? "" : "s"} selected
        </div>
      </Show>
